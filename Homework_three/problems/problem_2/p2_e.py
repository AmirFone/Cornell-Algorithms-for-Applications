def plan_city_e(num_data_hubs, num_service_providers, connections, provider_capacities, preliminary_assignment):
    ...
    if flow == 1:
        result = [0] * num_data_hubs
        for hub, provider in preliminary_assignment.items():
            result[hub] = provider
        for provider in graph[num_data_hubs - 1]:
            if graph[num_data_hubs - 1][provider] == 0:
                result[num_data_hubs - 1] = provider
    else:
        result = [0] * (num_data_hubs + num_service_providers)
        for provider in range(num_data_hubs, total_nodes + 1):
            if graph[sink_node].get(provider, 0) > 0:
                result[provider] += 1
